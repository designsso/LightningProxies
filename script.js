...
const moreinfo = (block) => {
    let info = document.querySelector(`#${block.parentNode.parentNode.id} .a`);
    let plus_minus = document.querySelector(`#${block.parentNode.parentNode.id} .icon`);

    if (boxes.hasOwnProperty(document.querySelector(`#${block.parentNode.parentNode.id}`)) == false) boxes[document.querySelector(`#${block.parentNode.parentNode.id} .icon`)] = false;
